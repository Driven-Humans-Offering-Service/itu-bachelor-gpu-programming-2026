...
#define DIV(x, y) (__fdiv_rn((x), (y)))
#define RCP(x) (__frcp_rn((x)))
#define MAF(x, y, z) (__fmaf_rn((x), (y), (z)))
...
__global__ void row_reduce_kernel(const float *__restrict__ input,
  ...
  if (BLOCK_SIZE >= 1024) {
    if (threadIdx.x < 512) {
      INC(sdata[threadIdx.x], sdata[threadIdx.x + 512]);
    }
    __syncthreads();
  }
  if (BLOCK_SIZE >= 512) {
    if (threadIdx.x < 256) {
      INC(sdata[threadIdx.x], sdata[threadIdx.x + 256]);
    }
    __syncthreads();
  }
  if (BLOCK_SIZE >= 256) {
    if (threadIdx.x < 128) {
      INC(sdata[threadIdx.x], sdata[threadIdx.x + 128]);
    }
    __syncthreads();
  }
  if (BLOCK_SIZE >= 128) {
    if (threadIdx.x < 64) {
      INC(sdata[threadIdx.x], sdata[threadIdx.x + 64]);
    }
    __syncthreads();
  }

  if (threadIdx.x < 32) {
    float val = ADD(sdata[threadIdx.x], sdata[threadIdx.x + 32]);

    for (int offset = 16; offset > 0; offset /= 2)
      INC(val, __shfl_down_sync(0xffffffff, val, offset));
...
__global__ void find_diag(float *alpha, float *beta, const float *a,
  ...
  if (j >= i) {
    if (i >= 1) {
      INC(sum, MUL(alpha[IDX(i, i - 1, N)], beta[IDX(j, i - 1, N)]));
    }
    beta[IDX(j, i, N)] = SUB(a[IDX(i, j, N)], sum);
  } else {
    if (j >= 1) {
      INC(sum, MUL(alpha[IDX(i, j - 1, N)], beta[IDX(j, j - 1, N)]));
    }
    alpha[IDX(i, j, N)] =
        MUL(RCP(beta[IDX(j, j, N)]), SUB(a[IDX(i, j, N)], sum));
  }
}
...
__global__ void multiply(float *alpha, float *beta, float *sum_matrix,
  ...
  if (y > x) {
    ...
    sum_matrix[IDX(y, x, N)] = MUL(alpha[IDX(i, x, N)], beta[IDX(j, x, N)]);
  }
}

// 0 FLOPs per thread
__global__ void fill(float *p, const int N, const float num) {
  int x = blockDim.x * blockIdx.x + threadIdx.x;
  int y = blockDim.y * blockIdx.y + threadIdx.y;
  if (x < N && y < N) {
    p[IDX(y, x, N)] = num;
...
__global__ void add_new_row(float *sum_array, float *alpha, float *y, int size,
  ...
  INC(sum_array[IDX(row, col, size)],
      MUL(alpha[IDX(row, i, size)], y[IDX(col, i, size)]));
}

// 2 FLOPs per thread
__global__ void add_new_row_bottom_up(float *sum_array, float *beta, float *x,
                                      int size, int i) {
  int col = blockDim.x * blockIdx.x + threadIdx.x;
  int row = blockDim.y * blockIdx.y + threadIdx.y;
  ...
  INC(sum_array[IDX(row, col, size)],
      MUL(beta[IDX(row, i, size)], x[IDX(col, i, size)]));
}

// 4 (i <= N - 2) or 2 (i == N - 1) FLOPs per thread
__global__ void findx(float *sum_array, float *beta, float *x, int size, int i,
                      float *y) {
  int col = blockDim.x * blockIdx.x + threadIdx.x;
  ...
  float sum = i <= size - 2
                  ? MAF(beta[IDX(i, i + 1, size)], x[IDX(col, i + 1, size)],
                        sum_array[IDX(i, col, size)])
                  : 0;

  x[IDX(col, i, size)] =
      DIV(SUB(y[IDX(col, i, size)], sum), beta[IDX(i, i, size)]);
}
...
__global__ void findy(float *sum_array, float *alpha, float *y, int size, int i,
  ...
  float sum = i >= 1 ? MAF(alpha[IDX(i, i - 1, size)], y[IDX(col, i - 1, size)],
                           sum_array[IDX(i, col, size)])
                     : 0;

  y[IDX(col, i, size)] =
      DIV(SUB(b[IDX(i, col, size)], sum), alpha[IDX(i, i, size)]);
}
