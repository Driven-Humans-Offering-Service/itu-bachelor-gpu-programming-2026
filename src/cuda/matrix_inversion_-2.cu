...
__global__ void inverse_matrix_kernel(float *a, float *res, float *alpha,
  ...
  for (int j = 0; j < N; j++) {
    for (int i = 0; i <= j; i++) {
      ...
      for (int k = 0; k < i; k++) {
        ...
        sum =
            __fadd_rn(__fmul_rn(alpha[IDX(i, k, N)], beta[IDX(j, k, N)]), sum);
      }
      beta[IDX(j, i, N)] = __fsub_rn(a[IDX(i, j, N)], sum);
    }
    for (int i = j + 1; i < N; i++) {
      float sum = 0;
      ...
      for (int k = 0; k < j; k++) {
        sum = __fadd_rn(__fmul_rn(alpha_p[k], beta_p[k]), sum);
      }
      alpha[IDX(i, j, N)] = __fmul_rn(__frcp_rn(beta[IDX(j, j, N)]),
                                      __fsub_rn(a[IDX(i, j, N)], sum));
    }
  }
  // transpose beta
  for (int i = 0; i < N; i++) {
  ...
  for (int col = 0; col < N; col++) {
    ...
    y[0] = __fdiv_rn(b[0], alpha[0]);
    for (int i = 1; i < N; i++) {
      float sum = 0.0f;
      for (int j = 0; j < i; j++) {
        sum = __fadd_rn(__fmul_rn(alpha[IDX(i, j, N)], y[j]), sum);
      }
      y[i] = __fdiv_rn(__fsub_rn(b[i], sum), alpha[IDX(i, i, N)]);
    }

    // findX
    x[N - 1] = __fdiv_rn(y[N - 1], beta[IDX(N - 1, N - 1, N)]);
    for (int i = N - 2; i >= 0; i--) {
      float sum = 0.0f;
      for (int j = i + 1; j < N; j++) {
        sum = __fadd_rn(__fmul_rn(beta[IDX(i, j, N)], x[j]), sum);
      }
      x[i] = __fdiv_rn(__fsub_rn(y[i], sum), beta[IDX(i, i, N)]);
    }
